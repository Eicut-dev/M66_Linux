 ...
 *     If input "Ql_GetMsSincePwrOn", that will get the number of milliseconds since the device booted.
 *     If input "Ql_Get_ExceptionInfo", that will get the exception information   
 *
 * Author:
 * -------
 * -------
...
static void proc_handle(char *pData,s32 len)
{
    ...
    //command: Ql_Get_ExceptionInfo
    if (Ql_strstr(pData,"Ql_Get_ExceptionInfo") != NULL)
    {
        u32 rdLen=0;
        u8 rdCnt = 0;
        u8 validCnt = 0;
        ...
        APP_DEBUG("start get exception information!\r\n");
        rdCnt = 5;
        Ql_GetExceptionRecords (exceptionBuf,&validCnt,rdCnt);
    
        APP_DEBUG("\r\n<--OpenCPU: get exception : %d, you want get: %d!-->\r\n",validCnt,rdCnt);  
                    
        if(validCnt)
        {
            for(i = 0;i < validCnt;i++)
            {
                APP_DEBUG("errorcode1: %x,errorcode2: %x\r\n",
                           exceptionBuf[i].content.fatalerr.error_code.code1,
                           exceptionBuf[i].content.fatalerr.error_code.code2);
            }
        }
        return;
    }
