...
s64   Ql_atoll (const char * s1);
char* Ql_strdup(const char* s1);
s32   Ql_vsprintf(char* s, const char* format , __gnuc_va_list arg);
s32   Ql_rand(void);
void  Ql_srand(u32 seed);
s32   Ql_vsnprintf(char* s, u32 n, const char* format,  __gnuc_va_list arg );
double Ql_strtod(const char *nptr,char **endptr);
